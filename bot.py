...
def handle_start(message):
    ...
    if result:
        ...
        if datetime.now() > expiration_date:
            ...
            vip_status = (
                f"✅ User VIP!\n"
                f"⏳ Days: {days_restantes} day(s)\n"
                f"📅 Expire en: {expiration_date.strftime('%d/%m/%Y %H:%M:%S')}"
            )
    else:
        vip_status = "❌ *Sorry You Not Vip.*"
...
def handle_ping(message):
    ...
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("⛔ NORMAL PING", callback_data=f"stop_{telegram_id}"))
